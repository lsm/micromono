...
Asset.prototype = {
  ...
  bundle: function(options, env) {
    ...
    options = options || {}
    var bundleCmd = 'bundle ' + path.join(this.getPublicPath(), jspmInfo.main)
    // bundleCmd = jspmInfo.main
    ...
    writeConfigSync(this.publicPath, options)
...
function writeConfigSync(publicPath, options) {
  ...
  var oldConfig = sandbox.oldConfig
  if (oldConfig) {
    // Note: the systemjs loader need `CSS` upper case.
    oldConfig.buildCSS = options.buildCss || false
    oldConfig.separateCSS = options.separateCss || false
    debug('update jspm config file: %s', configPath)
    fs.writeFileSync(configPath, 'System.config(' + JSON.stringify(oldConfig, null, 2) + ');')
  }
}
