...
exports.getJSPMConfig = function(assetInfo, publicPath, packagePath, next) {
  var configPath = path.join(publicPath, 'config.js')
  debug('get jspm config from %s', configPath)
  next.setDep('jspmConfigPath', configPath)
  fs.readFile(configPath, 'utf8', function(err, configCode) {
    ...
    next.setDep('jspmConfig', jspmConfig)
    next.setDep(exports.getJSPMBinPath(packagePath))
