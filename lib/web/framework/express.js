...
ExpressAdapter.prototype.startServer = function(port, host, service) {
  var mainApp
  var app = this.getApp()
  var baseUrl = service ? service.baseUrl || '/' : '/'
  var name = service && service.name

  if ('function' === typeof port) {
    debug('[%s] port is the main express app', name)
  ...
  this.mainApp.use(baseUrl, this.mapp)
  ...
  if (mainApp) {
    ...
    debug('[%s] listen to http requests at "%s:%s"', name, host, port)
    promise = new Promise(function(resolve, reject) {
      var server = mainApp.listen(port, host, function(error) {
        error ? reject(error) : resolve(server)
      })
    })
  } else if (service instanceof ServiceManager) {
    // not a service which means it's a balancer
    debug('[%s] binding http port should be handled externally', name)
    // let's get the server object
    mainApp = this.mainApp
    var _listen = mainApp.listen
    mainApp.listen = function(port, host) {
      debug('listen to requests at port %s host %s', port, host)
...
ExpressAdapter.prototype.attachRoute = function(route, router, service) {
  ...
  if (Array.isArray(route.middleware)) {
    ...
    this.app[method](routePath, middlewares, handler)
  } else {
    this.app[method](routePath, handler)
  }
