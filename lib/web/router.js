...
Router.prototype.normalizeRoutes = function(route, service) {
  ...
  Object.keys(route).forEach(function(routePath) {
    ...
    // We prefix the route path with `baseUrl` if it not starts with `^`
    if ('string' === typeof _route.path && '^' !== _route.path[0]) {
      _route.path = path.join(service.baseUrl, _route.path)
    }
