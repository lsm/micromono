...
var Router = module.exports = function MicroMonoRouter(service) {
  ...
  if (service.use) {
    this._uses = this.normalizeUses(service.use, service)
  }
...
/**
 * Get the internal use definition object. Could be used for service announcement.
 * @return {Object} Use definition object.
 */
Router.prototype.getUses = function() {
  return this._uses
}
...
Router.prototype.normalizeRoutes = function(route, service) {
  ...
  Object.keys(route).forEach(function(routePath) {
    ...
    if (service.isRemote()) {
      // We need to filter non-string value.
      if (Array.isArray(routeHandler.middleware)) {
        _route.middleware = routeHandler.middleware.filter(function(m) {
          // Only allow string which is the name of the middleware.
    ...
    if (_route.page) {
      ...
      var pageApiBaseUrl = service.pageApiBaseUrl || '/api'
      var apiRoutePath = path.join(pageApiBaseUrl, _route.path)
      _route.apiBaseUrl = pageApiBaseUrl
...
Router.prototype.normalizeUses = function(use, service) {
  var _uses = {}
  ...
  Object.keys(use).forEach(function(name) {
    ...
    _use = _use.map(function(url) {
      var _url = _formatRoutePath(url, service.baseUrl, 'default')
      return _url
    })
    _uses[name] = _use
  })
