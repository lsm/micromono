...
exports.addProvider = function(scheduler, ann) {
  ...
  debug('New provider for service [%s] found at "%s"', ann.name, ann.host)
  scheduler.add(ann)
}
  ...
  Object.keys(services).forEach(function(serviceName) {
    ...
    if (service.isRemote) {
      ...
      if (minInterval > ann.timeout)
        minInterval = ann.timeout > 500 ? ann.timeout : 500
    ...
    remoteServices.forEach(function(service) {
      ...
      scheduler.each(function(ann) {
        if (now - ann.lastSeen > ann.timeout) {
          debug('Provider "%s" of service [%s] timeout', ann.host, ann.name)
