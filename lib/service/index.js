...
exports.startService = function(micromono, Service, callback, dependencies) {
  ...
  if (service.channel) {
    servicePipeline = servicePipeline.concat(ChnPipeline.setupChannel)
    servicePipeline.pipe('startChannelServer', ['chnAdapter', 'chnEndpoint', 'next'], 'chnEndpoint')
  servicePipeline
    // Start web server
  ...
  servicePipeline(micromono.superpipe, service.name + ':startService')
}
