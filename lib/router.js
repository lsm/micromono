  ...
  buildRoutes: function() {
    ...
    Object.keys(routes).forEach(function(routePath) {
      ...
      if (_route.middleware) {
        routeApp[_route.method](_route.path, _route.middleware, _route.handler.bind(service));
      } else {
    ...
    if (!service.isRemote() && service.middleware) {
      ...
      Object.keys(middleware).forEach(function(name) {
        ...
        routeApp.all(path, function(req, res, next) {
          ...
          // record changes to `req` and `req.headers`
          ...
          middlewareFn(req, res, function(err) {
            ...
            if (semi) {
              // using a non-exists status code to indicate the middleware does
              // not need to change the response
              res.statusCode = 103;
