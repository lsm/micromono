...
Asset.prototype = {
  ...
  parseJSPM: function(packagePath) {
    ...
    if (pjson.jspm) {
      ...
      if (jspmInfo.directories) {
        publicURL = jspmInfo.directories.baseURL || publicURL;
        if (publicURL[0] !== '/') {
          publicURL = '/' + publicURL;
        }
        publicPath = jspmInfo.directories.lib || publicURL;
      }
  ...
  mergeJSPMDeps: function(jspmInfo) {
    ...
    Object.keys(depsToMerge).forEach(function(depName) {
      ...
      if (!oldDep) {
        ...
        console.warn('You have potential conflicting dependencies: %s vs %s', oldDep, dep);
      }
    });
