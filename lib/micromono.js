...
MicroMono.prototype = {
  ...
  runServer: function(app) {
    ...
    // @todo make this configurable by individual service
    if (_app.get('views')) {
      _app.use(partialRender(_app));
    }
...
function runService(micromono, app, ServiceFactory, callback) {
  var serviceInstance = new ServiceFactory();
  micromono.instances[serviceInstance.announcement.name] = serviceInstance;
  if (micromono.asset) {
    micromono.asset.mergeJSPMDeps(serviceInstance.announcement.client);
  }
