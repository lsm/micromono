...
SocketMQAdapter.prototype.connect = function(provider) {
  ...
  debug('Connecting to service [%s] endpoint "%s"', name, endpoint)
  ...
  if (!smq) {
    ...
    smq.on('disconnect', function(socket) {
      ...
      debug('Service [%s] provider "%s" disconnected', name, socket.provider.host)
    })
    smq.on('error', function(err) {
      var provider
      if (err.stream && err.stream.provider) {
        provider = err.stream.provider
        self.onProviderDisconnect(provider)
      }
      debug('Service [%s] provider "%s" error', name, provider ? provider.host : 'unknown host')
    })
  }
...
SocketMQAdapter.prototype.startServer = function(port, host, callback) {
  ...
  smq.on('connect', function(sock) {
    debug('Remote client connected from %s', sock.remoteAddress)
  })
