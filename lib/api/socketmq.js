...
SocketMQAdapter.prototype.connect = function(provider) {
  ...
  debug('Connecting to service [%s] API endpoint "%s"', name, endpoint)
  ...
  if (!smq) {
    ...
    smq.on('disconnect', function(socket) {
      ...
      debug('Service [%s] API provider "%s" disconnected', name, socket.provider.host)
    })
    smq.on('error', function(err) {
      var provider = err.stream && err.stream.provider
      debug('ERROR: service [%s] API provider "%s"', name, provider ? provider.host : 'unknown host')
    })
  }
...
SocketMQAdapter.prototype.startServer = function(port, host, callback) {
  ...
  smq.on('connect', function(stream) {
    debug('Remote client "%s" connected', stream.remoteAddress)
  })
  smq.on('disconnect', function (stream) {
    debug('Remote client "%s" disconnected', stream.remoteAddress)
  })
